...
def evaluate_metrics(csv_filepath: str):
    print(f"Loading LLM-judged results from {csv_filepath}...\n")
    ...
    metrics = {
        p: {
            ...
            "total_em": 0.0,
            "total_f1": 0.0,
            "total_llm": 0.0,  # LLM Judge binary scores (0/1 per question)
            "latencies": [],
            "ttfts": [],
            "loops": [],
            "errors": 0,
    ...
    with open(csv_filepath, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            for p_name, p_cfg in PIPELINE_CONFIG.items():
                ...
                p_llm = int(row.get(f'{prefix}_llm_score', 0) or 0)
                ...
                metrics[p_name]["total_llm"] += p_llm
    ...
    print("=" * 60)
    print("📊 FULL PIPELINE PERFORMANCE REPORT (5 Pipelines + LLM Judge)")
    print("=" * 60)
    ...
    rag_llm = (rag_m["total_llm"] / rag_valid) * 100
    ...
    for agent in ["linear_v1", "correction_v1", "linear_v2", "correction_v2"]:
        ...
        agent_llm = (am["total_llm"] / a_valid) * 100
        ...
        if lat_tax > 0:
            ...
            print(f"Efficiency:              {efficiency:+.4f}% LLM Accuracy per additional second")
        elif lat_tax < 0:
            print(f"Efficiency:              Faster than baseline! ({llm_gain:+.2f}% LLM Acc with {lat_tax:.2f}s less)")
        else:
...
if __name__ == "__main__":
    CSV_FILE = "./output/llm_judged_results_500_llama3.1.csv"
