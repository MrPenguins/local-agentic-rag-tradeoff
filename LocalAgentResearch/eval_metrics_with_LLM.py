...
def evaluate_metrics(csv_filepath: str):
    ...
    metrics = {
        p: {
            ...
            "total_em": 0.0,
            "total_f1": 0.0,
            "total_llm": 0.0,  # NEW: Tracker for LLM Judge scores
            "latencies": [],
            "ttfts": [],
            "loops": []
        } for p in pipelines
    ...
    with open(csv_filepath, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            # Safely parse LLM scores (default to 0 if missing or empty)
            rag_llm = int(row.get('rag_llm_score') or 0)
            lin_llm = int(row.get('linear_llm_score') or 0)
            cor_llm = int(row.get('correction_llm_score') or 0)
            ...
            for p_name, p_data in pipeline_data.items():
                ...
                # 2. Generation Metrics (EM, F1, LLM Judge)
                ...
                metrics[p_name]["total_llm"] += p_data["llm_score"]
    ...
    for p in pipelines:
        ...
        avg_llm = (metrics[p]["total_llm"] / total_questions) * 100
        ...
        print(f"LLM Judge Accuracy:          {avg_llm:.2f}%")
    ...
    rag_llm = (metrics["rag"]["total_llm"] / total_questions) * 100
    ...
    for agent in ["linear", "correction"]:
        ...
        agent_llm = (metrics[agent]["total_llm"] / total_questions) * 100
        ...
        if lat_tax > 0:
            efficiency = llm_gain / lat_tax
            print(f"Efficiency:         +{efficiency:.2f}% LLM Accuracy per additional second of Total Latency")
        else:
...
if __name__ == "__main__":
    # Ensure this points to the new CSV generated by your LLM Judge script
    CSV_FILE = "./output/llm_judged_results_500.csv"
